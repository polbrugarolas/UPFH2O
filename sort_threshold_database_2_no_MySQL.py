...
threshold = 100.0  # Consumo L/dia maximo definido
...
# 4 Filter rows where 'consum mitja L/dia' does not exceed the threshold
df_filtered = df_grouped[df_grouped['consum mitja L/dia'] <= threshold]


# 5 Prepare the new table with the required columns
new_table = df_filtered[['Secció sensal', 'total_contadors', 'consum mitja L/dia']]

# 6 Sort the data by 'consum mitja L/dia' in ascending order (lowest to highest)
new_table = new_table.sort_values(by='consum mitja L/dia', ascending=True)

# 7 Guardar la nueva tabla en un archivo CSV o Parquet
